...
The trading simulation plot below shows the balance (dotted brown line) and the total value (solid green line) of the trading account from the trading startdate to the enddate. The red and green triangles shown in the bottom of the plot show the trade enter end exit signals. The size of the trade position taken is determined by the position sizing strategy (here the risk taken per trade (1R) is 2% of the balance at the enter date). The trading account end balance is also compared against a configurable benchmark (optional), which is show as "Buy-and-Hold" (HODL). The HODL value is the value at the enddate of the benchmark, given the fact that 100% of the trading balance was used to buy the benchmark at the startdate, with no active trading in between the 2 dates. 
