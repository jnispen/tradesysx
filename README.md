...
A Monte Carlo simulation can be used to explore a range of **possible** alternative outcomes after obtaining a R-multiple distribution. However, the results from this simulation should be interpreted with caution as the simulation is inherently an approximation. The simulation attempts to visualize the range of possible alternate outcomes given the R-multiple distribution by drawing (*resampling*) with replacement from this distribution. By randomly drawing individual trade outcomes from the set, many different series of trade histories emerge while adhering to the distribution of the set. If an sufficient number of samples are drawn, the sampled average R-multiple will converge to the average R-multiple of the set.

For each simulated series of trades, the simulation samples a series with the length of the number of trades or less if `sim_len_max` is smaller and repeats this process N times (set by the `iterations` parameter). Furthermore, it is assumed that at each trade a 1R risk equal in size to the average risk calculated from the backtest results is taken. The average risk is taken in an attempt to simulate simultaneous trades, as will happen during real trading, in contrast to running the simulation as a series of completed sequential trades.
