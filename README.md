...
The Monte Carlo simulation plot can be used to explore a range of **possible** alternative outcomes after obtaining the R-multiple distribution from the backtest. However, the results from this simulation should be considered with caution as many restrictions and simplifications apply. The simulation basically tries to visualize the range of possible alternate outcomes given the R-multiple distribution by drawing (*resample*) with replacement from this distribution. By randomly drawing indivdual trade outcomes from the set, many different series of trade histories emerge, while keeping the same shape of the distribution and the same average R-multiple, as long as a sufficient number of samples are drawn.

The simulation samples a series with the lenght of the number of trades or less if `sim_len_max` is smaller, and repeats this process many times (set by the `iterations` parameter). Furthermore, it is assumed that each trade takes a 1R risk equal in size to the average risk calculated from the backtest results. This is done in an attempt to simulate simultanious trades, as will happen during real trading, in contrast to running the simulation as a series of completed individual trades.
