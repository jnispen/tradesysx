...
**stoploss** — [`float`]\
Multiplier of the close used as the stop when `stloss` is `percent` (e.g. `0.92` = stop 8% below the close). Ignored when `stloss` is `3atr`/`2atr`/`xatr`.

**atr_factor** — [`float`]\
ATR multiple below the close used as the stop when `stloss` is `xatr` (e.g. `2.5` = stop 2.5×ATR below the close). Ignored for the other `stloss` methods.
