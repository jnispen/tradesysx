...
- **Bollinger Bands (BBu / BBl)** — Upper/lower volatility bands around a moving average; price at the lower band = stretched down.
...
that justified the entry breaks down. `DONCH` exits on the opposite channel break. `RSI` and
...
The ladder only applies to the **`3EMA`, `SMA`, `MACD` and `DONCH`** exits. The other exits
